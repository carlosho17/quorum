...
var (
	// add Gauges so they can be exported to Influx as measurements
	// in Grafana the query would look like
	// SELECT "value" FROM "geth.chain/head/header.gauge" WHERE ("host" =~ /^$host$/) AND $timeFilter
...
func (bc *BlockChain) loadLastState() error {
	...
	headBlockGauge.Update(int64(currentBlock.NumberU64())) // added for Influx
	...
	headFastBlockGauge.Update(int64(currentBlock.NumberU64())) // added for Influx
	if head := rawdb.ReadHeadFastBlockHash(bc.db); head != (common.Hash{}) {
		if block := bc.GetBlockByHash(head); block != nil {
			bc.currentFastBlock.Store(block)
			headFastBlockGauge.Update(int64(block.NumberU64())) // added for Influx
		}
	}
...
func (bc *BlockChain) FastSyncCommitHead(hash common.Hash) error {
	...
	headBlockGauge.Update(int64(block.NumberU64())) // added for Influx
	bc.mu.Unlock()
...
func (bc *BlockChain) ResetWithGenesisBlock(genesis *types.Block) error {
	...
	bc.currentBlock.Store(bc.genesisBlock)
	headBlockGauge.Update(int64(bc.genesisBlock.NumberU64())) // added for Influx
	bc.hc.SetGenesis(bc.genesisBlock.Header())
	bc.hc.SetCurrentHeader(bc.genesisBlock.Header())
	bc.currentFastBlock.Store(bc.genesisBlock)
	headFastBlockGauge.Update(int64(bc.genesisBlock.NumberU64())) // added for Influx
...
func (bc *BlockChain) Rollback(chain []common.Hash) {
	...
	for i := len(chain) - 1; i >= 0; i-- {
		...
		if currentFastBlock := bc.CurrentFastBlock(); currentFastBlock.Hash() == hash {
			...
			headFastBlockGauge.Update(int64(newFastBlock.NumberU64())) // added for Influx
		}
		if currentBlock := bc.CurrentBlock(); currentBlock.Hash() == hash {
			newBlock := bc.GetBlock(currentBlock.ParentHash(), currentBlock.NumberU64()-1)
			bc.currentBlock.Store(newBlock)
			headBlockGauge.Update(int64(newBlock.NumberU64())) // added for Influx
